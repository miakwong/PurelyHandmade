...
function initUserProfile() {
  ...
  console.log('User data retrieved from localStorage:', currentUser);
  ...
  if (currentUser) {
    ...
    // Add debug log after updating user information
...
function checkAdminStatus() {
  ...
  // Retrieve and check current user data
  const currentUser = JSON.parse(localStorage.getItem('currentUser'));
  ...
  // Retrieve and check admin-tab-container element
  const adminTabContainer = document.getElementById('admin-tab-container');
  console.log('Admin button container element:', {
    element: adminTabContainer,
    exists: !!adminTabContainer,
    display: adminTabContainer?.style.display,
    className: adminTabContainer?.className
  });
  ...
  if (adminTabContainer) {
    ...
    // Check various possible admin flags
    const isAdminUser = currentUser && (
      currentUser.isAdmin === true || 
      currentUser.isAdmin === 1 ||  // Add check for number 1
      currentUser.role === "admin" || 
      currentUser.role === "administrator" ||
      currentUser.admin === true ||
      currentUser.admin === 1  // Add check for number 1
    );
    ...
    console.log('Admin permission check result:', {
      ...
      conditions: {
        hasUser: !!currentUser,
        isAdminFlag: currentUser?.isAdmin === true || currentUser?.isAdmin === 1,
        roleAdmin: currentUser?.role === "admin",
        roleAdministrator: currentUser?.role === "administrator",
        adminFlag: currentUser?.admin === true || currentUser?.admin === 1
      }
    });

    if (isAdminUser) {
      adminTabContainer.style.display = 'block';
      console.log('Showing admin dashboard button - user is admin');
    } else {
      console.log('Hiding admin dashboard button - user is not admin or not logged in');
    }
  } else {
    console.error('Admin button container element (admin-tab-container) not found');
  }
